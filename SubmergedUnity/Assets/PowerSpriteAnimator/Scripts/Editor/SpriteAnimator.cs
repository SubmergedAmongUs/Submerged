...
public partial class SpriteAnimator : EditorWindow 
{
	...
	[SerializeField] string[] m_defaultNodeNames = new string[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
	[SerializeField] Sprite[] m_defaultNodeSprites = new Sprite[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
	...
	void OnEnable()
	{
		...
		for ( int i = 0; i < SpriteAnimNodes_POWERTOOLS.NUM_NODES; ++i )
		{
	...
	void OnClipChange( bool resetPreview = true )
	{
		...
		for ( int i = 0; i < events.Length; ++i )
		{
			...
			newEvent.m_sendUpwards = newEvent.m_functionName.Length > 0 && newEvent.m_functionName[0] == SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_NOPARAM[0];
			...
			if ( newEvent.m_sendUpwards )
			{
				...
				if ( newEvent.m_functionName.Length <= SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_NOPARAM.Length )
				{
					newEvent.m_paramType = eAnimEventParameter.None;
				}
				else 
				{
					switch( newEvent.m_functionName[SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_NOPARAM.Length] )
					{
				...
				switch (newEvent.m_paramType)
				{
					...
					{
						newEvent.m_functionName = newEvent.m_paramString;
						newEvent.m_paramInt = SpriteAnimEventHandler_POWERTOOLS.EventParser.ParseInt(ref newEvent.m_functionName);
						newEvent.m_paramString = null;
					} break;
					case eAnimEventParameter.Float:
					{
						newEvent.m_functionName = newEvent.m_paramString;
						newEvent.m_paramFloat = SpriteAnimEventHandler_POWERTOOLS.EventParser.ParseFloat(ref newEvent.m_functionName);
						newEvent.m_paramString = null;
					} break;
					case eAnimEventParameter.String:
					{
						newEvent.m_functionName = newEvent.m_paramString;
						newEvent.m_paramString = SpriteAnimEventHandler_POWERTOOLS.EventParser.ParseString(ref newEvent.m_functionName);
					} break;
		...
		for ( int i = 0; i < m_events.Count; ++i )
		{
			...
			if ( from.m_sendUpwards )
			{
				...
				switch( from.m_paramType )
				{
					...
					case eAnimEventParameter.None:
					{
						to.functionName = SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_NOPARAM;
						to.stringParameter = functionName;
					} break;
					case eAnimEventParameter.Int:
					{
						to.functionName = SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_INT;
						to.stringParameter = functionName + SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_DELIMITER + from.m_paramInt.ToString();
					} break;
					case eAnimEventParameter.Float:
					{
						to.functionName = SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_FLOAT;
						to.stringParameter = functionName + SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_DELIMITER + from.m_paramFloat.ToString();
					} break;
					case eAnimEventParameter.String:
					{
						to.functionName = SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_STRING;
						to.stringParameter = functionName + SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_DELIMITER + from.m_paramString;
					} break;
					case eAnimEventParameter.Object:
					{	
						// Object events are split into two. The first is the string function name, second is object name					
						to.functionName = SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_OBJECT_FUNCNAME;
						to.stringParameter = functionName;
						...
						to.time = from.m_time;
						to.messageOptions = from.m_messageOptions;
						to.functionName = SpriteAnimEventHandler_POWERTOOLS.EventParser.MESSAGE_OBJECT_DATA;
