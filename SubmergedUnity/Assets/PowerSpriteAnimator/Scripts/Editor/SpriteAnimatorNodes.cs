...
public partial class SpriteAnimator
{
	...
	static readonly System.Type TYPE_SPRITEANIMNODES = typeof(SpriteAnimNodes_POWERTOOLS);
	...
	[SerializeField] Vector2[] m_nodePositions = new Vector2[SpriteAnimNodes_POWERTOOLS.NUM_NODES]; // positions of the nodes
	[SerializeField] float[] m_nodeAngles = new float[SpriteAnimNodes_POWERTOOLS.NUM_NODES]; // angles of the nodes
	...
	void LayoutAnimationNodesPanel( Rect rect )
	{
		...
		for ( int i = SpriteAnimNodes_POWERTOOLS.NUM_NODES-1; i >= 0; --i ) 
		{
			if ( i != m_selectedNode  && Utils.BitMask.IsSet(m_visibleNodes,i) )
	...
	{		
		...
		if ( m_nodePositions == null || m_nodePositions.Length == 0 )
			m_nodePositions = new Vector2[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
		if ( m_nodeAngles == null || m_nodeAngles.Length == 0 )
			m_nodeAngles = new float[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
	...
	void ReadNodesFromClip()
	{
		...
		for ( int nodeId = 0; nodeId < SpriteAnimNodes_POWERTOOLS.NUM_NODES; ++nodeId )
		{
	...
	void ApplyNodeChanges()
	{
		...
		AnimationCurve[] curvesX = new AnimationCurve[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
		AnimationCurve[] curvesY = new AnimationCurve[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
		AnimationCurve[] curvesAngle = new AnimationCurve[SpriteAnimNodes_POWERTOOLS.NUM_NODES];

		// Cached node values to check 
		float[] prevValueX = new float[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
		float[] prevValueY = new float[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
		float[] prevValueAngle = new float[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
	...
	void CreateStaticBindings()
	{
		...
		{
			NODE_BINDINGS_X = new EditorCurveBinding[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
			NODE_BINDINGS_Y = new EditorCurveBinding[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
			NODE_BINDINGS_ANGLE = new EditorCurveBinding[SpriteAnimNodes_POWERTOOLS.NUM_NODES];
